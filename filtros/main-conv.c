...
void rx_heterodinacao(double *u, double *i, double *q, int length,
    ...
    //printf("tam: %d\n",length);
    //imprime_vetor_to_double(u,length);
    ...
    //printf("tamanho: %d\n",length);
    //imprime_vetor_to_double(i,length);
    //passagem pelo filtro
    i = rrc(i,k,length,N);
    q = rrc(q,k,length,N);
    ptr->i  = i;
    ptr->q  = q;
    //imprime_vetor_to_double(i,length+N-1);
...
*/
    // testa convolução 
    int h_leng = 3,vet_leng = 4, tam_convolu = h_leng + vet_leng -1;
    double h[] = {2,2,2}, vet[]={3,7,2,9}, *buffert = zera_buffer(h_leng);
    double *resultado = aloca_memoria_double(tam_convolu);

    resultado = convolucao(vet,h,h_leng,buffert,resultado,tam_convolu,false);
    imprime_vetor_to_double(resultado,tam_convolu);
    plota_constelacao(resultado,resultado,tam_convolu,"16-QAM");

    imprime_vetor_to_double(&resultado[h_leng-1],vet_leng-h_leng+1);
